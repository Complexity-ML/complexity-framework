...
Same parameter count as train_400m_v1.py (~354M) but WITHOUT Token-Routed MLP,
Zipf routing, Shared Expert, or Mu-Guidance. Pure dense transformer baseline.
...
logger = logging.getLogger("train_400m_dense")
...
# ── Model config (~484M params, dense) ──────────────────────────────────

def make_config() -> ModelConfig:
    """Dense baseline: iso-params with Token-Routed MoE (full shared expert).
    hidden=1024, layers=20, heads=16, kv_heads=4, inter=5888, dense → ~484M.
    """
    return ModelConfig(
        hidden_size=1024,
...
def main():
    parser = argparse.ArgumentParser(description="Train 400M Dense Baseline (iso-params comparison)")
    ...
    parser.add_argument("--checkpoint-dir", type=str, default="./checkpoints/400m-dense")
    ...
    if is_main:
        ...
        logger.info(f"  inter={config.intermediate_size}, dense SwiGLU (no MoE)")
    ...
    if is_main:
        if args.wandb:
            wandb_cb = WandBCallback(project=args.wandb, name="400m-dense")
            trainer.callbacks.append(wandb_cb)

        tqdm_cb = TqdmCallback(total_steps=max_steps, desc="400M Dense")
