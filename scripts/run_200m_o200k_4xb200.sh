...
DENSE_CONFIG="$REPO_ROOT/configs/run_configs/200m_o200k_chinchilla/dense_gqa_seed42_4b_b200.yaml"
TR_CONFIG="$REPO_ROOT/configs/run_configs/200m_o200k_chinchilla/tr_gqa_fixed_id_seed42_4b_b200.yaml"
...
run_dense() {
  ...
  "$PYTHON_BIN" -m torch.distributed.run --standalone --nproc_per_node="$NPROC" \
    ...
    --save-dir "$ARTIFACT_ROOT/checkpoints/dense-gqa-200m-o200k-4b-s42" \
    2>&1 | tee "$ARTIFACT_ROOT/logs/dense-gqa-200m-o200k-4b-s42.log"
}
...
run_tr() {
  ...
  "$PYTHON_BIN" -m torch.distributed.run --standalone --nproc_per_node="$NPROC" \
    ...
    --tokens-path "$DATA_ROOT/train" \
    --eval-tokens-path "$DATA_ROOT/eval" \
    --save-dir "$ARTIFACT_ROOT/checkpoints/tr-gqa-fixed-id-200m-o200k-4b-s42" \
    2>&1 | tee "$ARTIFACT_ROOT/logs/tr-gqa-fixed-id-200m-o200k-4b-s42.log"
}
...
smoke_dense() {
  ...
  "$PYTHON_BIN" -m torch.distributed.run --standalone --nproc_per_node="$NPROC" \
    ...
    --eval-batches 2 \
    --save-steps 0 \
    --run-name smoke-dense-gqa-200m-o200k-4b-s42 \
    --save-dir "$ARTIFACT_ROOT/checkpoints/smoke-dense-gqa-200m-o200k-4b-s42" \
    2>&1 | tee "$ARTIFACT_ROOT/logs/smoke-dense-gqa-200m-o200k-4b-s42.log"
}
...
case "${1:-}" in
  ...
  collect-dense)
    collect_run \
      "dense-gqa-200m-o200k-4b-s42" \
      "dense-gqa-200m-o200k-4b-s42" \
      "dense-gqa-200m-o200k-4b-s42"
    ;;
  collect-tr)
    collect_run \
      "tr-gqa-fixed-id-200m-o200k-4b-s42" \
      "tr-gqa-fixed-id-200m-o200k-4b-s42" \
      "tr-gqa-fixed-id-200m-o200k-4b-s42"
    ;;
  *)
