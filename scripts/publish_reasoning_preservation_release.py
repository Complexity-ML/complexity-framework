#!/usr/bin/env python3
"""Publish and verify the selected reasoning-preservation root bundle."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--repo-id",
        default="AETHORIA-AI/TR-HASH-MoE-200M-160B-Reasoning-Preservation-50M",
    )
    ...
    manifest = json.loads((args.bundle / "release_manifest.json").read_text())
    weights = args.bundle / "model.safetensors"
    local_sha = sha256(weights)
    if manifest.get("weights_dtype") != "float32":
        raise RuntimeError("Root release must use F32 SafeTensors")
    if local_sha != manifest.get("weights_sha256"):
        raise RuntimeError("Local weights do not match release manifest")
    ...
    api.upload_folder(
        ...
        path_in_repo=".",
        token=token,
        commit_message=f"Promote preservation step {manifest['selected_step']}",
    )
    required = {
        "README.md",
        "config.json",
    ...
    if missing:
        raise RuntimeError(f"Remote release is incomplete: {missing}")
    ...
    if config.get("model_type") != "tr_hash_moe":
        raise RuntimeError("Remote config lost the TR-HASH model type")
