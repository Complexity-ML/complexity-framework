...
def make_config(mlp_type: str = "dense_deterministic") -> ModelConfig:
    """70M dense config — embeddings 32k×640 + 10 layers of GQA(10/2) + SwiGLU(1920)."""
    return ModelConfig(
        hidden_size=640,
        ...
        attention_type="gqa",
        mlp_type=mlp_type,
        intermediate_size=1920,  # ~3× hidden → ~70M total
        num_experts=1,
        shared_expert=False,
        norm_type="rmsnorm",
        use_qk_norm=True,
...
def main():
    ...
    pbar = tqdm(total=args.steps, desc="train dense", unit="step", dynamic_ncols=True)
