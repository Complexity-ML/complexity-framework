"""
Local ~50M residual Token-Routed run with o200k tokenizer.

Runs on CUDA, MPS, or CPU with the same CLI/log schema as the 300M scaling
script. This profile is sized around 50M parameters after the large o200k
embedding table is included.

Examples:
    python3 scripts/train_50m_o200k_tr_local.py --steps 100 --dataset random
    python3 scripts/train_50m_o200k_tr_local.py --dataset text --text-file README.md --tokenizer ./tokenizer-o200k
"""
...
logger = logging.getLogger("train_50m_o200k_tr_local")
...
def main():
    parser = argparse.ArgumentParser(description="Local ~50M o200k residual Token-Routed run")
    ...
    parser.add_argument("--shared-intermediate-size", type=int, default=1024)
    ...
    parser.add_argument("--run-name", type=str, default="50m-o200k-tr-local")
    parser.add_argument("--label-smoothing", type=float, default=0.0)
    parser.add_argument("--z-loss", type=float, default=0.0)
    parser.add_argument("--save-steps", type=int, default=5000)
    parser.add_argument("--save-dir", type=str, default="checkpoints/50m-o200k-tr-local")
    ...
    if is_main:
        ...
        logger.info(
            "Config: Token-Routed residual, hidden=224, layers=8, GQA=7/1, "
    ...
    model.train()
    pbar = (
        tqdm(total=args.steps, initial=start_step, desc="50M o200k TR", unit="step", dynamic_ncols=True)
