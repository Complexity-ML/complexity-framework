...
Usage:
    # Single node, 8× B300
    torchrun --nproc_per_node=8 scripts/train_adamtr_383m.py

    # Multi-node (e.g. 2 nodes × 8 GPU)
    torchrun --nnodes=2 --nproc_per_node=8 \\
        --master_addr=$MASTER_ADDR --master_port=29500 \\
        --node_rank=$NODE_RANK \\
        scripts/train_adamtr_383m.py

    # Resume
    torchrun --nproc_per_node=8 scripts/train_adamtr_383m.py --resume checkpoints/adamtr-383m/step_5000
...
logger = logging.getLogger("adamtr-383m")
...
def main():
    parser = argparse.ArgumentParser(description="AdamTR 383M Token-Routed + Spectral Conditioning")
    ...
    parser.add_argument("--adam-lr", type=float, default=3e-4,
                        help="AdamTR base learning rate")
    ...
    parser.add_argument("--checkpoint-dir", type=str, default="./checkpoints/adamtr-383m")
    ...
    if is_main:
        logger.info(f"=== AdamTR 383M Token-Routed + Spectral Conditioning ===")
    ...
    # Trainer — AdamTR (per-expert spectral conditioning) + ClusterModel
    ...
    if is_main:
        if args.wandb:
            wandb_cb = WandBCallback(project=args.wandb, name="adamtr-383m")
            trainer.callbacks.append(wandb_cb)

        tqdm_cb = TqdmCallback(total_steps=max_steps, desc="AdamTR 383M")
