...
if (( CPU_COUNT > 8 )); then
  ...
  DEFAULT_RAYON_THREADS="${CPU_COUNT}"
fi
...
if [[ "${TR_HASH_125B_CREATE_PRIVATE_REPO:-1}" == "1" ]]; then
  args+=(--create-private-repo)
fi

cd "${REPO_ROOT}"
exec "${PYTHON_BIN}" -m scripts.build_tr_hash_pretraining_125b "${args[@]}" "$@"
