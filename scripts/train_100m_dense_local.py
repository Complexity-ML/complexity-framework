"""
Local ~100M dense SwiGLU baseline.

Runs on CUDA, MPS, or CPU with a tiny local loop and CSV logging. Use this as
the dense counterpart to train_100m_tr_local.py.

Examples:
    python3 scripts/train_100m_dense_local.py --steps 20 --dataset random
    python3 scripts/train_100m_dense_local.py --dataset text --text-file data/sample.txt --tokenizer ./tokenizer --bf16
"""
...
logger = logging.getLogger("train_100m_dense_local")
...
def main():
    ...
    parser.add_argument("--run-name", type=str, default="100m-dense-local")
