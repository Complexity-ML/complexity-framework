"""
Local ~100M Token-Routed + Mu-Guidance run.

Runs on CUDA, MPS, or CPU with the same CLI/log schema as the dense local
baseline. This is the quick A/B counterpart to train_100m_dense_local.py.
...
def main():
    parser = argparse.ArgumentParser(description="Local ~100M Token-Routed run")
    ...
    parser.add_argument("--routed-gate-init", type=float, default=1.0)
    parser.add_argument("--learn-shared-routed-gates", action="store_true")
    ...
    logger.info(
        "Config: Token-Routed + Mu, hidden=640, layers=10, GQA=10/2, "
    ...
    model.train()
    pbar = tqdm(total=args.steps, desc="100M TR", unit="step", dynamic_ncols=True)
