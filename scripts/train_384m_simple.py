"""
Train 384M ComplexityModel on Apple Silicon (MPS) / CPU fallback.

Token-Routed MoE + GQA + SwiGLU — ~384M params.

Usage:
    python scripts/train_384m_mps.py
    python scripts/train_384m_mps.py --steps 500 --batch-size 8 --bf16 --grad-ckpt
...
logger = logging.getLogger("train_384m")
...
# ---------------------------------------------------------------------------
# Config: ~384M params (same as train_400m_v1 but simple training loop)
# hidden=1024, 20 layers, GQA 16h/4kv, Token-Routed MoE x4, Mu-Guidance
# ---------------------------------------------------------------------------
...
def main():
    parser = argparse.ArgumentParser(description="Train 384M on MPS/CPU")
    ...
    pbar = tqdm(total=args.steps, desc="train 384M", unit="step", dynamic_ncols=True)
