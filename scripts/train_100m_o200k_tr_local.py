"""
Local ~100M residual Token-Routed run with o200k tokenizer.
...
Examples:
    ...
    python3 scripts/train_100m_o200k_tr_local.py --dataset text --text-file README.md --tokenizer ./tokenizer-o200k
"""
...
logger = logging.getLogger("train_100m_o200k_tr_local")
...
def main():
    parser = argparse.ArgumentParser(description="Local ~100M o200k residual Token-Routed run")
    ...
    parser.add_argument("--shared-intermediate-size", type=int, default=1536)
    ...
    if is_main:
        ...
        logger.info(
            "Config: Token-Routed residual, hidden=384, layers=10, GQA=8/2, "
    ...
    model.train()
    pbar = (
        tqdm(total=args.steps, initial=start_step, desc="100M o200k TR", unit="step", dynamic_ncols=True)
