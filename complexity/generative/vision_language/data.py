"""Streaming reader for image + prompt/response WebDataset shards."""
...
class VisionLanguageTarDataset(IterableDataset):
    """Read ``.webp/.jpg/.png + .txt + .json`` samples without extracting TAR shards.

    ``.txt`` holds the target response. ``.json`` metadata may carry a
    ``"prompt"`` field; when absent, ``default_prompt`` is used -- this lets a
    plain image-caption corpus (stage 1: fixed "Describe this image." prompt,
    per docs/tr-hash-image-text-to-text.md) and a real image-grounded
    question/answer corpus (stage 2) share one reader.
    """
                ...
                for member in archive:
                    ...
                    if image_suffix and "txt" in row and "json" in row:
                        ...
                        yield {
                            ...
                            "prompt": str(metadata.get("prompt") or self.default_prompt),
                            "response": row["txt"].decode("utf-8").strip(),
                            "metadata": metadata,
                        }
                        del pending[sample_id]


def collate_vision_language(rows: List[Dict[str, object]]) -> Dict[str, object]:
    if not rows:
        raise ValueError("cannot collate an empty vision-language batch")
    return {
