...
def parse_args() -> argparse.Namespace:
    ...
    source.add_argument(
        "--hf-dataset",
        help="Hugging Face image-classification dataset identifier",
    )
...
def main() -> None:
    ...
    if distributed.is_main:
        ...
        LOGGER.info(
            "TR-Hash vision backend: %s (experts=%d top_k=%d shared_width=%d "
            "expert_width=%d active_width=%d stored_width=%d)",
            backend["selected_backend"],
            backend["experts"],
            backend["top_k"],
            backend["shared_width"],
            backend["expert_width"],
            backend["active_width"],
            backend["stored_width"],
        )
