...
# softplus(_TARGET_GATE_INIT) / (softplus(_TARGET_GATE_INIT) + softplus(_CONTEXT_GATE_INIT))
# lands within 1e-3 of the (1, 0) split that unnormalized fusion gets for free from
# zero-initialized gates, so normalized fusion keeps the same identity-at-init
# transfer contract instead of blending target/context 50/50 (softplus(0) is equal
# for both branches) from the first forward pass.
_TARGET_GATE_INIT = 4.0
_CONTEXT_GATE_INIT = -8.0


def _fusion_gate(levels: int, channels: int, normalized_fusion: bool) -> nn.Parameter:
    count = max(levels - 1, 0)
    if not normalized_fusion:
        return nn.Parameter(torch.zeros(count))
    gates = torch.empty(count, 2, channels)
    gates[:, 0, :] = _TARGET_GATE_INIT
    gates[:, 1, :] = _CONTEXT_GATE_INIT
    return nn.Parameter(gates)
...
class CrossScaleFusionNeck(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.normalized_fusion = normalized_fusion
        self.laterals = nn.ModuleList(_identity_channel_projection(channels) for _ in range(levels))
        self.top_down_gates = _fusion_gate(levels, channels, normalized_fusion)
        ...
        if mode == "pan":
            ...
            self.bottom_up_gates = _fusion_gate(levels, channels, normalized_fusion)
    ...
    def forward(self, feature_maps: list[torch.Tensor]) -> list[torch.Tensor]:
        ...
        for level in range(len(top_down) - 2, -1, -1):
            ...
            fused = self._fuse(top_down[level], context, self.top_down_gates[level])
        ...
        for level, (downsample, refinement) in enumerate(
            ...
            fused = self._fuse(target, context, self.bottom_up_gates[level])
    ...
    def _fuse(
        ...
        if not self.normalized_fusion:
            return target + gates * context
        weights = F.softplus(gates)
        weights = weights / weights.sum(dim=0, keepdim=True).clamp_min(1e-6)
        target_weight, context_weight = weights.unbind(dim=0)
        shape = (1, self.channels, 1, 1)
        return target_weight.view(shape) * target + context_weight.view(shape) * context
