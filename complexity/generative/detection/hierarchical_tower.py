...
class HierarchicalTRHashBlock(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.attention = _SpatialSelfAttention(hidden_size, num_heads, dropout)
...
class HierarchicalTRHashVisionTower(nn.Module):
    ...
    def __init__(self, config):
        ...
        max_grids = [
            math.ceil(self.max_base_grid / (2**stage))
        ...
        stages = []
        layer_index = 0
        for stage_index, (depth, max_grid) in enumerate(
            zip(config.vision_stage_depths, max_grids)
        ):
