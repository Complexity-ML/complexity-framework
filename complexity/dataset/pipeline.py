...
class DataPipeline:
    ...
    def _pack(self, tokenized: List[List[int]], seq_len: int) -> List[List[int]]:
        ...
        # Pretraining packing is a continuous token stream.  This preserves
        # every token, crosses document boundaries, and splits long documents
        # instead of truncating their tail.
        stream = [token for sequence in tokenized for token in sequence]
        return [stream[offset : offset + seq_len] for offset in range(0, len(stream), seq_len)]
