        ...
        if config.dp_size > 1:
            if config.use_ddp:
                ...
                model = DDP(
                    ...
                    # MoE models have parameters that may not see gradients on
                    # every forward (for example an expert receiving no token).
                    # DDP needs unused-parameter detection in that case.
