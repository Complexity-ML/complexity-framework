...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, config: ModelConfig, layer_idx: int = 0):
        ...
        mlp_config = MLPConfig(
            ...
            bigram_keys=getattr(config, 'bigram_keys', None),
            bigram_experts=getattr(config, 'bigram_experts', None),
            bigram_bos_id=getattr(config, 'bigram_bos_id', 0),
            ctx_sig_keys=getattr(config, 'ctx_sig_keys', None),
            ctx_sig_experts=getattr(config, 'ctx_sig_experts', None),
            token_class_table=getattr(config, 'token_class_table', None),
            ctx_window=getattr(config, 'ctx_window', 0),
            ctx_num_buckets=getattr(config, 'ctx_num_buckets', 0),
