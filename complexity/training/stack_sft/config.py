...
class StackSFTConfig:
    ...
    lora_targets: str = "q_proj,v_proj,o_proj,shared_gate,shared_up,shared_down"
    ...
    def from_dict(cls, data: dict[str, Any]) -> "StackSFTConfig":
        ...
        if lora_rank <= 0:
            raise ValueError("stack SFT requires a positive LoRA rank")
        return cls(
            name=str(data["name"]),
            ...
            lora_targets=str(
                data.get(
                    "lora_targets",
                    "q_proj,v_proj,o_proj,shared_gate,shared_up,shared_down",
                )
            ),
            stages=stages,
        )
