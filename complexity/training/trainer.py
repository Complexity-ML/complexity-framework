...
class Trainer:
    ...
    def __init__(
        ...
        if self.is_main:
            ...
            distributed_mode = "fsdp" if config.use_fsdp else ("ddp" if self.distributed else "single")
            logger.info(f"  distributed     : {distributed_mode}    sharding: {config.sharding_mode}")
    ...
    def train(self) -> Dict[str, Any]:
        ...
        if self.config.resume_from:
            ...
            for callback in self.callbacks:
                on_resume = getattr(callback, "on_resume", None)
                if on_resume is not None:
                    on_resume(self.global_step)
        ...
        try:
            while self.global_step < self.config.max_steps:
                ...
                for batch_idx, batch in enumerate(self.train_dataloader):
                    ...
                    with self._gradient_sync_context(should_sync=should_sync):
    ...
    def _gradient_sync_context(self, *, should_sync: bool):
        """Skip distributed gradient all-reduce on accumulation micro-batches."""
    ...
    def _snapshot_canary_params(self):
        ...
        attention) so the assertion check is O(few) tensors,
