"""
Dataset API - Flexible avec defaults.
"""
...
class Dataset:
    ...
    def load(cls, path: Union[str, Path], tokenizer=None, config: DataConfig = None, **kwargs) -> "Dataset":
        """
        Charge dataset. Override tout via kwargs.

        Args:
            path: Fichier ou dossier
            tokenizer: Tokenizer (optionnel)
            config: Config (optionnel)
            **kwargs: Override (seq_length, text_column, ...)
        """
    ...
    def from_config(cls, config: Union[Dict, DataConfig], tokenizer=None, **kwargs) -> "Dataset":
        """Charge depuis config dict (style YAML)."""
    ...
    def streaming(cls, path: Union[str, Path], tokenizer=None, **kwargs) -> "StreamingDataset":
        """Dataset streaming pour gros fichiers. kwargs override defaults."""
        config = DataConfig(**kwargs)
    ...
    def batches(self, **kwargs) -> Iterator[Dict[str, torch.Tensor]]:
        """Itère par batches. kwargs override (batch_size, shuffle, ...)."""
        loader = self.get_dataloader(**kwargs)
        for batch in loader:
            yield batch

    def get_dataloader(self, **kwargs) -> DataLoader:
        """DataLoader PyTorch. kwargs override defaults."""
    ...
    def filter(self, fn: Callable[[Dict], bool], **kwargs) -> "Dataset":
        """Filtre. kwargs passés au nouveau Dataset."""
    ...
    def split(self, ratio: float = 0.1, seed: int = None, **kwargs) -> tuple["Dataset", "Dataset"]:
        """Split train/val. kwargs passés aux nouveaux Datasets."""
        import random
        data = self._data.copy()
        if seed:
            random.seed(seed)
    ...
    @classmethod
    def create_pipeline(cls, data_path, output_path, tokenizer, **kwargs) -> "DataPipeline":
        """Pipeline de préparation. kwargs override defaults."""
        return DataPipeline(Path(data_path), Path(output_path), tokenizer, **kwargs)
...
class DataPipeline:
    ...
    def _pack(self, tokenized, seq_len):
        packed, current = [], []
