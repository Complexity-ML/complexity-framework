...
\begin{align}
    ...
    \mV &= \mH\mW_v.
\end{align}
Le modèle évalué utilise huit têtes de lecture et deux têtes partagées d'écriture/valeur, de dimension 48. Lectures et écritures reçoivent des normalisations RMS indépendantes par tête et un encodage rotatif. Les têtes W/V sont répétées par groupes de quatre têtes R, puis l'attention causale est
\begin{equation}
    \vy_{t,h}=\sum_{j\le t}
    ...
    \right)\vv_{j,\kappa(h)}.
\end{equation}
PyTorch SDPA est configuré pour autoriser le dispatch FlashAttention sur H200. W/R/V est donc bien un mécanisme d'attention : R et W occupent respectivement la frontière technique query/key de SDPA, tandis que V reste contextuel. La terminologie décrit le flux, non un opérateur sans attention. En décodage incrémental, les tenseurs W et V passés sont conservés et croissent avec le préfixe. Un résidu lexical d'écriture exploratoire existe dans l'implémentation, mais il est fixé à zéro et omis dans tous les runs contrôlés du tableau~\ref{tab:h200-controlled}.
