...
if HAS_TRITON:
    ...
    def _pair_coverage_hash_kernel(
        ...
        pair_index_mask: tl.constexpr,
        swap_mask: tl.constexpr,
        ...
        pair_indices = route_codes & pair_index_mask
    ...
    def _pair_hash_block_counts_kernel(
        ...
        pair_indices = codes & pair_index_mask
        expert_a = tl.load(
...
def _pair_code_masks(pair_count: int) -> tuple[int, int]:
    """Return backward-compatible pair-index and orientation masks."""

    if not 1 <= pair_count <= 32:
        raise ValueError("compact pair metadata supports one to 32 pairs")
    return (0x7, 0x8) if pair_count <= 8 else (0x1F, 0x20)
...
def pair_coverage_hash_expert_ids(
    ...
    *,
    ...
    pair_index_mask, swap_mask = _pair_code_masks(int(expert_pairs.size(0)))
    ...
    if HAS_TRITON and flat_token_ids.is_cuda:
        ...
        _pair_coverage_hash_kernel[
            ...
            pair_index_mask=pair_index_mask,
            swap_mask=swap_mask,
            BLOCK_SIZE=block_size,
        )
        return output.view(2, *original_shape)
