...
two to eight experts. That path compiles each vocabulary route into one byte,
...
2. **Implemented:** hash-native fused CUDA path for top-2 and up to eight
...
4. **Next:** generalize the hash-native partition workspace to 16 experts and
   top-1/top-4, then capture it inside CUDA Graph buckets.
