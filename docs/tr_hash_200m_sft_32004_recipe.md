...
Supervisor configuration is generated by the framework API rather than stored
as instance-specific `.conf` files:

```python
from complexity.training import SupervisorManager, tr_hash_sft_32004_program
...
Installation is atomic, paths and program names are validated, and control
commands use argv without a shell. Secret-like environment keys are rejected;
credentials remain in protected instance state and never enter a generated
configuration or Git history. `autostart=true` plus
`autorestart=unexpected` restarts a crashed job without looping after a clean
completion.
