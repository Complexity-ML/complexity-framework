...
\begin{enumerate}
  ...
  \item an auditable accounting of 129.996B base exposures, 32.069B refinement exposures, and 238.9M supervised SFT tokens;
...
\end{equation}
Thus the routed contribution is the sum of the two selected expert outputs. The shared branch remains active for every token and is substantially wider than each expert. Stored intermediate width is $3072+4\times256=4096$; active intermediate width is $3072+2\times256=3584$. Expert tensors account for 5.47\% of model parameters.
...
\begin{tikzpicture}[font=\small,>=stealth]
  ...
  \foreach \x/\c/\title/\sub in {
    ...
    11.1/trviolet/{Full SFT}/{238.9M supervised tokens}}
  {
  ...
  \node[draw=trviolet,rounded corners=2pt,fill=trviolet!8,align=center,font=\scriptsize] at (13.6,1.15) {epoch 2 promoted\\69.31\% PIQA acc\_norm};
...
Instruction post-training starts from the step-8,156 refinement checkpoint. The model is trained for three epochs over the Luciole 16-way instruction mixture, totaling 238.9M supervised tokens. The mixture covers instruction following, conversation, reasoning, code, mathematics, science, safety, retrieval, rewriting, and summarization. The \code{complexity-chat-v2} template is used for SFT and chat serving. All model parameters are optimized; this stage is not LoRA or QLoRA.
...
Epoch 3 & 1,389 & \textbf{1.220861} & \textbf{3.39} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table*}[t]
\centering
\caption{PIQA validation results for the released lineage. ``Source exposure'' excludes the 238.9M supervised SFT tokens. No significance test or multi-seed interval is claimed.}
\label{tab:piqa}
\small
...
Full SFT, epoch 2 & 162.065B + SFT & \textbf{68.82\%} & \textbf{69.31\%} & Promoted root assistant checkpoint \\
\bottomrule
\end{tabularx}
\end{table*}
...
\begin{center}
\scriptsize\code{2dd5a35624dda4cb267d872f974fd448}\\[-1pt]
\code{c9e085004c163b31772515bfd9573caa}.
\end{center}
The release is available at \url{https://huggingface.co/AETHORIA-AI/TR-HASH-MoE-200M-160B-SFT}; the base and refinement repositories retain their distinct roles. The public chat at \url{https://www.complexity-ai.fr/ai-lab} loads the promoted full-SFT model. Experimental LoRA repositories are not part of this lineage.
