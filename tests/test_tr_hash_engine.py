...
def test_route_tables_are_deterministic_distinct_and_balanced_enough(
    ...
    for route_index in range(top_k):
        ...
        tolerance = 1 if strategy not in {
            TRHashStrategy.AFFINE_HASH,
            TRHashStrategy.MULTI_HASH,
        } else 80
...
def test_compact_pair_metadata_rejects_unsupported_shape():
    ...
    with pytest.raises(ValueError, match="two to eight"):
        compile_top2_pair_metadata(routes, num_experts=16)
...
def test_multi_hash_routes_depend_on_every_hash_channel_and_layer():
    common = dict(
        hidden_size=8,
        vocab_size=4_093,
        num_experts=4,
    ...
    two_hashes = build_route_table(
        TRHashEngineConfig(**common, route_hash_count=2, layer_index=0)
    )
    three_hashes = build_route_table(
        TRHashEngineConfig(**common, route_hash_count=3, layer_index=0)
    )
    next_layer = build_route_table(
        TRHashEngineConfig(**common, route_hash_count=2, layer_index=1)
    )

    assert not torch.equal(two_hashes, three_hashes)
    assert not torch.equal(two_hashes, next_layer)
    assert torch.all(two_hashes[0] != two_hashes[1])
...
def test_fused_cuda_matches_reference_forward_and_gradients(num_experts):
