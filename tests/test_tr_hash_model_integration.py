...
def test_model_can_enable_multi_hash_without_changing_expert_capacity():
    config = _config()
    config.routing_strategy = "token_id_multi_hash"
    config.route_hash_count = 3
    model = ComplexityModel(config)

    for layer in model.layers:
        engine = layer.mlp.engine
        assert engine.config.num_experts == 4
        assert engine.config.top_k == 2
        assert engine.config.route_hash_count == 3
        assert engine.config.routing_strategy.value == "token_id_multi_hash"
        assert torch.all(engine.route_table[0] != engine.route_table[1])
