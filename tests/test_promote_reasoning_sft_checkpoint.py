...
def test_promotes_final_when_reasoning_improves_and_retention_holds(tmp_path) -> None:
    ...
    promoted, _, _ = promote(
        ...
        {"combined": {"acc": 0.50, "acc_norm": 0.50}},
    )
...
def test_accepts_reasoning_gain_when_normalized_arc_is_retained(tmp_path) -> None:
    first = str(tmp_path / "step_001000")
    final = str(tmp_path / "step_004724")
    candidates = [
        {
    ...
    promoted, _, _ = promote(
        summary,
        [_reasoning(first, 0.10), _reasoning(final, 0.12)],
        [
            {"checkpoint": first, "combined": {"acc": 0.49, "acc_norm": 0.47}},
            {"checkpoint": final, "combined": {"acc": 0.478, "acc_norm": 0.458}},
        ],
        {"combined": {"acc": 0.50, "acc_norm": 0.462}},
    )
    assert promoted["selected"]["step"] == 4724
