...
def test_remap_hf_state_dict_adds_mlx_prefix_and_primary_hash_route():
    routes = torch.tensor([[0, 1, 2, 3], [2, 3, 0, 1]], dtype=torch.int64)
    state = {
        "layers.0.mlp.topk_token_to_expert": routes,
        "layers.0.mlp.gate_proj_w": torch.randn(4, 16, 2, dtype=torch.bfloat16),
    }

    converted = remap_hf_state_dict(state, dtype="float16")
    ...
    assert converted["model.layers.0.mlp.topk_token_to_expert"].dtype == torch.int32
    assert torch.equal(
        converted["model.layers.0.mlp.token_to_expert"], routes[0].to(torch.int32)
    )
    assert converted["model.layers.0.mlp.gate_proj_w"].dtype == torch.float16
